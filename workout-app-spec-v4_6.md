...
**문서 성격**
- v4.6은 Slice 2 완성 + 실사용 경험을 반영한 **사용자 피드백 통합 버전**이다.
...
**v4.5 대비 주요 변경사항**
- **같은 세션 내 이전 세트 복사 기능 명시** (F1.15)
  - 같은 무게/횟수의 반복 입력 비효율 해소
  - 본세트/워밍업 구분에 따른 복사 동작 규칙 명시
- **커스텀 종목 추가 UI 명시** (F2 확장)
  - 시드 데이터에 없는 종목을 사용자가 앱 내에서 직접 추가
  - 입력 부담 최소화 (필수 필드 4개, 나머지는 자동 추정)
  - `muscle_mapping_confidence` 플래그로 추정 매핑 표시
- **백업 권장 알림 기능 추가** (F8 확장)
  - 5회 이상 세션 + 백업 미시행 시 자동 안내
  - 30일 이상 백업 미시행 시 재안내
- **UX 색상 시스템 표준화** (부록 7.7 신규)
  - 의미-색 일치 원칙 (위험=빨강, 진행=초록, 정보=파랑)
  - 다크 모드 친화적 채도 가이드
- 7.5 체크리스트에 위 4개 항목 반영

**v4.1~v4.5에서 유지된 주요 변경사항**
- 진행 중 세션 표현 (`duration_seconds === null`) + 자동 복구 (v4.5)
- 종목 삭제 (SessionExercise cascade) (v4.5)
- 동시 편집 충돌 방지 (`sessionStore` 분리) (v4.5)
- 영구 저장 권한 요청 (`navigator.storage.persist()`) (v4.5)
- `Exercise / ExerciseVariation / alternative_group_id` 경계 정리 (v4.3)
- weighted bodyweight 처리 (v4.3)
- 안전 필터는 안전 상한선만 적용 (v4.3)
- 피로도 `over_limit`에서 progression 차단 (v4.3)
- 통증 기록 없음 vs 명시적 무통증 체크인 구분 (v4.3)
- 4.0 공용 유틸리티 + variation fallback (v4.2)
- `alternative_group_id` 채우기 규칙, 부상 프로필 자동 상태 조정 (v4.2)
- `effective_sets`와 `fatigue_score`의 역할 분리 (v4.1)
- 워밍업 처리 규칙 정합성 (v4.1)
- `UserExerciseVariationStats`로 사용자별 통계 분리 (v4.1)
- `PainLog`에 `user_id` 직접 추가 (v4.1)
- `BodyRegion` 표준 테이블 (v4.1)
- 맨몸/추가중량/어시스트 운동 모델 (v4.1)
- 4.0 유틸리티 보강, `set_load_kg()` 기반 부하 조회, 안전 필터 fallback 체인, 무통증 체크인 UX (v4.4)
...
- **F1.15** 같은 세션 내 이전 세트 복사 (v4.6)
  - 세트 추가 시 기본값은 빈 입력 (기존 동작 유지)
  - 별도 버튼(예: ⟲ "이전 세트와 동일")으로 직전 세트 값 복사 가능
  - 복사 동작 규칙:
    - 본세트 → 본세트: 무게, 횟수, RPE/RIR 모두 복사
    - 워밍업 → 워밍업: 무게, 횟수 복사 (RPE/RIR은 미복사)
    - 워밍업 → 본세트: 복사 버튼 비활성화 (의미적으로 부적절)
    - 직전 세트 없음 (첫 세트): 복사 버튼 비활성화
  - 복사 후 사용자가 일부 값 수정 가능 (예: 횟수만 수정)
  - 점진적 과부하 추천(Phase 2)이 들어와도 이 기능은 유지: 추천 무게 ≠ 직전 무게인 경우가 많아 복사 기능은 별도 가치
...
- **F2.12** 커스텀 종목 추가 UI (v4.6)
  - 시드 데이터에 없는 종목을 사용자가 앱 내에서 직접 추가
  - 입력 부담 최소화 — 필수 필드 4개:
    - 종목 이름 (자유 텍스트)
    - 카테고리 (compound / isolation 중 선택)
    - 기구 (barbell / dumbbell / machine / cable / bodyweight / ez_bar 중 선택)
    - 주동근 1개 (Muscle 드롭다운에서 선택)
  - 선택 입력:
    - 보조근 0~2개 + 자극 계수 (0.5 또는 0.25)
    - 권장 휴식 시간 (카테고리 기반 기본값 자동 제공)
    - 권장 반복 범위 (카테고리 기반 기본값 자동 제공)
  - 자동 추정/기본값 적용 (사용자에게 안 물어봄):
    - `weight_increment_kg`: 컴파운드 2.5, 아이솔레이션 1.25
    - `fatigue_factor`: 1.0
    - `recommendation_bias`: 1.0
    - `joint_stress_region_ids`: 기구 + 카테고리 기반 자동 추정 (또는 비워둠)
    - `movement_patterns`: 자동 추정 (또는 비워둠)
    - `alternative_group_id`: null (Phase 3에서 사용자 매칭 또는 자동 추정)
    - `skill_level`, `stability`, `unilateral`: 기본값
  - **`muscle_mapping_confidence: 'user_estimated'`** 로 설정 (시드 종목은 `'verified'`)
  - 위치: 종목 목록 화면 우상단에 "+" 버튼 → 모달
- **F2.13** 커스텀 종목 수정/삭제 (v4.6)
  - 사용자가 추가한 종목(`is_custom: true`)만 수정/삭제 가능
  - 시드 종목은 수정 불가 (UI에서 수정 버튼 비활성화)
  - 사용된 종목 삭제 시도 시: 과거 세션 데이터 보존을 위해 soft delete 또는 사용자에게 경고
    - 권장: 삭제 대신 "보관" 상태로 전환 (목록에서 숨김, 과거 데이터는 유지)
...
- 선택적 클라우드 동기화
- **백업 권장 알림** (v4.6)
  - 5회 이상 세션 기록 + 백업 미시행 시 홈/히스토리 화면 상단에 안내 카드 표시
  - 카드 내용: "데이터를 안전하게 보관하기 위해 백업을 권장합니다."
  - 카드 클릭 시 설정 → 백업 화면으로 이동
  - 백업 1회 완료 시 안내 카드 사라짐
  - 마지막 백업 후 30일 경과 시 다시 표시
  - User에 마지막 백업 시점 저장 필드 추가: `last_backup_at: timestamp | null`
...
```typescript
{
  ...
  /**
   * v4.6 신규: 보조근 매핑 정확도 플래그
   * - 'verified': 시드 데이터 또는 사용자 검증 완료
   * - 'user_estimated': 사용자가 직접 추가했지만 보조근 매핑 미검증
   * Phase 2 추천 엔진에서 'user_estimated' 종목은 부위별 볼륨 계산 시 주의 처리
   */
...
27. **같은 세션 내 이전 세트 복사 버튼을 제공한다** (v4.6)
    - 기본값은 빈 입력, 별도 버튼으로 복사.
    - 본세트/워밍업 구분에 따라 복사 동작 달라짐.
    - F1.15 참고.

28. **커스텀 종목은 자동 추정 + 최소 입력으로 받는다** (v4.6)
    - 필수 4개: 이름, 카테고리, 기구, 주동근.
    - 보조근은 0~2개 선택.
    - 나머지는 자동 추정.
    - `muscle_mapping_confidence: 'user_estimated'`로 표시.
    - F2.12 참고.

29. **백업 권장 알림을 명시적으로 노출한다** (v4.6)
    - 5회 이상 세션 + 미백업 시 상단 안내.
    - 30일 이상 미백업 시 재안내.
    - F8 참고.

30. **색상은 의미와 일치시킨다** (v4.6)
    - 빨강 = 위험/종료, 초록 = 진행/추가, 파랑 = 정보/저장.
    - 다크 모드 친화적 채도 (Tailwind 400~500 단계).
    - 한 화면 3가지 색 이하.
    - 부록 7.7 참고.

31. **SPA 라우팅 폴백 설정 누락 주의** (v4.6)
    - 정적 호스팅(Vercel 등)에서 `vercel.json` 또는 동등한 폴백 설정 필요.
    - 없으면 루트 외 경로에서 404 발생.
...
### 7.7 UX 색상 시스템 가이드 (v4.6 신규)

**배경**

검은 바탕에 흰 글씨만으로 구성된 UI는 헬스장에서 빠르게 인지하기 어렵다. 특히 액션 버튼(세트 추가, 종목 추가, 세션 종료 등)이 모두 같은 색이면 한 손으로 빠르게 누를 때 실수가 발생할 수 있다.

**핵심 원칙**

**의미와 색을 일치시킨다.** 사용자가 색만 보고 어떤 동작인지 인지할 수 있어야 한다.

| 의미 | 색상 카테고리 | Tailwind 권장 (다크 모드) |
|------|--------------|---------------------------|
| 위험 / 종료 / 삭제 | 빨강 계열 | `red-400`, `red-500` |
| 진행 / 추가 / 시작 | 초록 계열 | `emerald-400`, `green-500` |
| 정보 / 기본 / 저장 | 파랑 계열 | `sky-400`, `blue-500` |
| 경고 / 주의 | 노랑/주황 계열 | `amber-400`, `orange-400` |
| 비활성 / 보조 | 회색 계열 | `gray-400`, `slate-400` |

**다크 모드 친화적 채도**

- 원색(saturated primary)은 피한다. 톤 다운된 색을 사용한다.
- Tailwind 기준: `500`보다 `400` 단계가 다크 배경에 더 부드럽다.
- 텍스트는 흰색 또는 거의 흰색으로.

**한 화면 색상 가짓수**

- 한 화면에 너무 많은 색을 쓰면 오히려 혼란을 부른다.
- 권장: 한 화면에 색상 카테고리 3가지 이하 (배경/텍스트 제외).

**구체 적용 예시 (세션 화면)**

- "세트 추가" 버튼: 초록 (`emerald-400`)
- "종목 추가" 버튼: 초록 (`emerald-400`) — 같은 의미라 같은 색
- "세션 종료" 버튼: 빨강 (`red-400`)
- "세트/종목 삭제" 버튼: 빨강 (`red-400`)
- "저장" 또는 "확인" 버튼: 파랑 (`sky-400`)
- "취소" 버튼: 회색 (`gray-400`)

**부상 안전 관련 (Phase 3 이후)**

부상 부위, 통증 알림 등에 색을 활용할 때:
- 부상 부위 표시: 노랑(주의) 또는 주황(경고) — 빨강은 "위험/긴급"이라 너무 강함
- 통증 3/5 이상: 빨강 (긴급)
- 통증 1~2/5: 노랑 (주의)
- 무통증 체크인: 초록 (안전)
...
- 색맹 사용자를 위해 색 단독으로 의미 전달하지 않기 (아이콘, 텍스트 병행)
- 예: "삭제" 버튼은 빨강 + 휴지통 아이콘 + "삭제" 텍스트
