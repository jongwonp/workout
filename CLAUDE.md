...
**현재 단계**: Phase 1 / Slice 2.5 — 메모 앱 대체 수준 기능 완성. Slice 1·2 완료, Slice 2.5(일상 사용성 개선: 이전 세트 복사, 커스텀 종목 추가, 색상 시스템) 진행. 추천 엔진은 Phase 2 이후.
...
- `workout-app-spec-v4_6.md`: 전체 spec (최종 지향 설계, 최신 버전). **참고용. 현재 슬라이스 범위 외 기능을 만들지 말 것.** (v4.5 이하 구버전은 삭제됨)
- `v4.6-changes.md`: Slice 2 실사용 후 v4.6 변경 결정 메모

### 현재 슬라이스 작업 문서

- `slice2.5-spec.md`: 현재 작업 중인 슬라이스의 상세 명세. **작업 시작 전 필수 정독.**
- `slice1-spec.md`, `slice2-spec.md`: 완료된 이전 슬라이스 명세 (참고용)
...
- `slice3-spec.md`: 통증/부상 입력 슬라이스 명세 (미정)
- `v4.7-changes.md`: Slice 2.5 실사용 후 변경 결정 메모 (작성됨, 누적 중)
- `README.md`: 사용자용 설치/실행 가이드 (작성됨)
...
정적 호스팅 배포 시 vercel.json 필요

### 10. 색상은 의미와 일치 (위험=빨강, 진행=초록, 정보=파랑).
